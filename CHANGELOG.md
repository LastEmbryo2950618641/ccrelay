...
- 新增 Center 集中管理固定 Prompt 的目录能力，支持 `prompt install/list/remove` 以及 `UNIFIED`、`PRE`、`POST` 三类有序 Prompt。
- 新增 Relay Prompt revision 原子同步、任务级不可变快照与本地 SHA-256 复核；同步不完整或内容损坏时阻断新任务，避免节点使用混合版本。
- 新增 POST 私有二次定稿流程：候选回复不进入共享上下文或观察事件，工具与协作在定稿阶段禁用，只有第二阶段成功结果对外发布。
...
- Relay 固定职责始终位于用户 UNIFIED Prompt 之前；UNIFIED 变化时旋转私有模型 Session 并重放共享最终回复，PRE 在每个任务正式 ReAct 前按幂等键注入。
