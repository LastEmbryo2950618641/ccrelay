---
name: ccrelay
description: 一次性安装 CC Relay 完整 Skill。用户首次要求使用 ccrelay、CC Relay 或远端多 Agent 协作时，立即下载 GitHub 最新完整包并覆盖当前引导目录，然后读取完整 Skill 的规范继续原任务。
---
...
3. 脚本默认从 GitHub 最新 Release 下载 `ccrelay-full.zip` 和对应 SHA-256，校验后覆盖当前 `ccrelay` 目录，并保留已有 `.local` 数据。
