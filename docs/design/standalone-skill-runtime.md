...
目标节点已有同版本健康 Relay 时，计划应标记 `ALREADY_READY`，不得覆盖或再次传输。实际进入 Relay 部署任务后默认执行幂等覆盖：精准停止目标端口对应的 Relay，删除产品生成的目标工作目录和失败残留，再重新分发制品；该语义在引导的执行方式确认阶段提示，不增加中途确认。不得删除 SSH 凭据、专用账号、集群密钥、节点互信或产品目录之外的用户文件。计划取消时只取消尚未开始的子任务，并向运行中的源/中心任务发送合作式停止请求，最后记录未完成目标清单。
...
SSH 参数不再假定所有节点使用相同 OpenSSH 版本。凭据支持 `DEFAULT` 和 `USER_PROVIDED` 两种参数来源；默认模式使用兼容 OpenSSH 7.4 的 `StrictHostKeyChecking=no`，后者保存用户提供的 argv token，并在本地 SSH、SCP 及该节点发起的远端互信检查中使用，不叠加 Skill 自己的连接、HostKey、BatchMode 或认证参数，仅保留目标端口、地址和必要的密钥参数。

首次 SSH 引导采用独立于 Relay 部署扩散调度的有界并发模型。`bootstrap next`、`ssh identity plan|select|apply|verify|rotate-key` 和 `center plan|bootstrap` 统一接受 `--concurrency`，默认 `4`、范围 `1-32`。节点连接与环境探测、账号或公钥安装、Center 到节点验证按节点并发；FULL_MESH 按有向信任边并发。单节点内步骤、共享密钥生成、专用密码持久化和最终配置写入保持串行；结果按输入顺序汇总，单节点异常只影响自身结果。该参数不替代部署扩散的全局/单源并发门禁。

专用账号探测必须返回 `dedicatedInspection=VERIFIED|UNMANAGED|INACCESSIBLE`。只有 `UNMANAGED` 才能在权限明确时判定同名账号冲突；`INACCESSIBLE` 只能表示引导账号无法读取专用账号目录。若专用账号的集群密钥或受保护密码能够登录，验收直接使用专用账号执行，不得把缺少 `sudo -u` 权限解释为账号创建失败。
...
8. `replaceExistingRelay` 默认为 `true`。部署器必须在覆盖制品目录前停止旧 Relay，并删除产品生成的目标工作目录后重建。进程匹配必须同时包含 `RemoteCcRelayServer` 和精确目标端口，并使用绝对系统命令路径，禁止按端口杀死未知业务进程；制品归档不得携带运行时生成的 Claude Skill 符号链接。
...
- 状态：完整 P7 调度器尚未实施；当前已补充面向中小集群的 CLI 批量提交能力，不得将其标记为完整 P7 生产能力。
- 当前最小能力：`task create-batch` 在 CLI 层展开并去重目标，复用现有单目标 `DEPLOY_RELAY` 执行器，共享一次部署会话，以有界并发创建独立子任务；不新增 Java 任务表或改变单目标接口。
- 当前批量能力的失败隔离边界：提交阶段和中心 SSH 预检按目标返回结果，单目标任务创建失败不取消已提交或尚未提交的其他目标；部署终态、进度和重试仍由各子任务独立管理。
- 当前批量能力不包含：动态源池、滚动波次、目标租约、自动换源、计划级取消、计划级重启恢复和完整父任务聚合。
